# TAKEN FROM https://github.com/olivierhagolle/peps_download AND CUSTOMIZED
    ...
    if options.extract and os.path.exists(zfile):
        try:
            with zipfile.ZipFile(zfile, "r") as zf:
                ...
                name = safename[:-5]
                print(name)
                if not os.path.exists(os.path.join(options.write_dir, name)):
                    os.mkdir(os.path.join(options.write_dir, name))
                zf.extractall(os.path.join(options.write_dir, name))
            safedir = os.path.join(options.write_dir, name, safename)
        ...
        for i in range(len(data["features"])):
            ...
            try:
                ...
                if storage == "unknown":
                    ...
                    print(
                        "please send and email with product name to peps admin team : exppeps@cnes.fr"
                    )
                else:
...
if len(sys.argv) == 1:
    ...
    print("        ou : ", prog, " -h")
    print(
        "example 1 : python %s -l 'Toulouse' -a peps.txt -d 2016-12-06 -f 2017-02-01 -c S2ST"
        % sys.argv[0]
    )
    print(
        "example 2 : python %s --lon 1 --lat 44 -a peps.txt -d 2015-11-01 -f 2015-12-01 -c S2"
        % sys.argv[0]
    )
    print(
    ...
    print(
        "example 4 : python %s -l 'Toulouse' -a peps.txt -c SpotWorldHeritage -p SPOT4 -d 2005-11-01 -f 2006-12-01"
        % sys.argv[0]
    )
    print(
        "example 5 : python %s -c S1 -p GRD -l 'Toulouse' -a peps.txt -d 2015-11-01 -f 2015-12-01"
        % sys.argv[0]
    )
    sys.exit(-1)
else:
    ...
    parser.add_option(
        ...
        action="store",
        default="VV",
        help="VV,VV+VH,HH,HH+HV",
    )
    parser.add_option(
        "--processing",
        dest="processing_level",
        type="string",
        action="store",
        default="LEVEL1",
        help="LEVEL1,LEVEL2,LEVEL22",
    )
    parser.add_option(
        "--orbit_direction",
...
if options.tile is None:
    if options.location is None:
        ...
        if (
            ...
            and (options.latmax is None)
            and (options.lonmax is None)
            and (options.lat is None)
            or (options.lon is None)
        ):
            geom = "location"
        else:
...
if options.collection == "S2":
    if options.start_date >= "2016-12-05":
        print(
            "**** products after '2016-12-05' are stored in Tiled products collection"
        )
...
if options.collection == "S2ST":
    if options.end_date < "2016-12-05":
        print(
            "**** products before '2016-12-05' are stored in non-tiled products collection"
        )
...
if (options.product_type == "") and (options.sensor_mode == ""):
    search_catalog = (
        "curl -k -o %s https://peps.cnes.fr/resto/api/collections/%s/search.json?%s\&startDate=%s\&completionDate=%s\&maxRecords=500"
        % (
            options.search_json_file,
            options.collection,
            query_geom,
            start_date,
            end_date,
    ...
    search_catalog = (
        "curl -k -o %s https://peps.cnes.fr/resto/api/collections/%s/search.json?%s\&startDate=%s\&completionDate=%s\&maxRecords=500\&productType=%s\&sensorMode=%s\&polarisation=%s\&processingLevel=%s\&orbitDirection=%s"
        % (
            options.search_json_file,
            options.collection,
            query_geom,
...
if len(download_dict) == 0:
    ...
    for prod in list(download_dict.keys()):
        file_exists = os.path.exists(
            ("%s/%s.SAFE") % (options.write_dir, prod)
        ...
        if not (options.no_download) and not (file_exists):
            if storage_dict[prod] == "tape":
                ...
                get_product = (
                    'curl -o %s -k -u "%s:%s" https://peps.cnes.fr/resto/collections/%s/%s/download/?issuerId=peps &>/dev/null'
    ...
    while NbProdsToDownload > 0:
        ...
        prod, download_dict, storage_dict, size_dict = parse_catalog(
            options.search_json_file
        )
        ...
        for prod in list(download_dict.keys()):
            file_exists = (
                os.path.exists(("%s/%s.SAFE") % (options.write_dir, prod))
                or os.path.exists(("%s/%s.zip") % (options.write_dir, prod))
                or os.path.exists(("%s/%s") % (options.write_dir, prod))
            )
            if not (options.no_download) and not (file_exists):
                if storage_dict[prod] == "disk":
                    ...
                    get_product = (
                        'curl -o %s -k -u "%s:%s" https://peps.cnes.fr/resto/collections/%s/%s/download/?issuerId=peps'
                        % (
                            tmpfile,
                            email,
                            passwd,
                    ...
                    if not os.path.exists(
                        ("%s/tmp_%s.tmp") % (options.write_dir, tmticks)
                    ):
                        NbProdsToDownload += 1
                    else:
        ...
        if NbProdsToDownload > 0:
            ...
            print(
                "%d remaining products are on tape, lets's wait 1 minute before trying again"
                % NbProdsToDownload
            )
            print(
                "##############################################################################"
